<script>
	...
	let sizeDiscr = SETTINGS.MIN_SYMBOL_HOLDER; //sets a default value
	let incr =
		(SETTINGS.MAX_SYMBOL_HOLDER - SETTINGS.MIN_SYMBOL_HOLDER) /
		Object.keys(SETTINGS.DISCRETE_BREAKPOINTS).length;
</script>
...
<style>
	...
	:global(:is(.media-M, .media-L, .media-XL)) .picker-wrapper {
